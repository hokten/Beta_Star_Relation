...
%============================ Bas ============================
\usepackage[maxnames=2,bibstyle=numeric]{biblatex}
...
\author{\begin{tabular}[t]{c}
    ...
    Department of Ondokuz Mayıs University, 55139 Samsun, Turkey \\
...
\thispagestyle{empty}
\begin{abstract}
  In this paper, we generalize $ \beta_* $ relation on submodules of a module to elements of a complete modular lattice. Let $ L $ is 
  a complete modular lattice. We say $ a,b \in L $ are $ \beta_* \ equilavent $, $ a \beta_* b $, if and only if 
  for each $ t \in L $ such that $ a \vee t = 1 $ then $ b \vee t = 1 $ and for each $ k \in L $ such that $ b \vee k = 1 $ then $ a \vee k = 1 $. 
  We show that the $ \beta_* $ relation is an equivalence relation. Then, we examine $ \beta_* $ relation on weak supplemented lattices. 
  Finally, we show that $ L $ is weak supplemented if and only if for every $ x \in L $, $ x $ is equilavent to a weak supplement in $ L $. 
  \quad \\
  \quad \\

  \Sinif{06C05, 16D10} \\
  \Keywords{$ \beta_* $-relation, weak supplement, weakly supplemented lattice, complemented lattice, amply supplemented lattice, hollow lattice.
  }
\end{abstract}
